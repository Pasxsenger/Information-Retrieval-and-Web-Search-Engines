...
with open("visit_latimes.csv", "r", encoding="UTF-8") as f:
    data = pd.read_csv(f, header=0)
    total_urls_extracted = data["Number of outlinks found"].sum()
    less_1KB = data[data["Size(Byte)"] < 1024].shape[0]
    less_10KB = data[(1024 <= data["Size(Byte)"]) & (data["Size(Byte)"] < 10 * 1024)].shape[0]
    less_100KB = data[(10 * 1024 <= data["Size(Byte)"]) & (data["Size(Byte)"] < 100 * 1024)].shape[0]
    less_1mb = data[(100 * 1024 <= data["Size(Byte)"]) & (data["Size(Byte)"] < 1024 * 1024)].shape[0]
    greater_1mb = data[1024 * 1024 <= data["Size(Byte)"]].shape[0]
...
with open("urls_latimes.csv", "r", encoding="UTF-8") as f:
    ...
    unique_within = data[data["Indicator"] == "OK"].shape[0]
    unique_outside = data[data["Indicator"] == "N_OK"].shape[0]

with open("CrawlReport_latimes.txt", "w") as f:
    f.write(f"Name: Xuesong Shen\n")
    f.write(f"USC ID: 8138053893\n")
